...
const PRINCIPLES = [
  {
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <rect x="2" y="2" width="7" height="7" rx="1.5" fill="var(--sq-brand)" opacity="0.9"/>
        <rect x="11" y="2" width="7" height="7" rx="1.5" fill="var(--sq-brand)" opacity="0.55"/>
        <rect x="2" y="11" width="7" height="7" rx="1.5" fill="var(--sq-brand)" opacity="0.55"/>
        <rect x="11" y="11" width="7" height="7" rx="1.5" fill="var(--sq-brand)" opacity="0.25"/>
      </svg>
    ),
    title: 'Consistencia',
  ...
  },
  {
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <circle cx="10" cy="10" r="8" stroke="var(--sq-brand)" strokeWidth="1.5" fill="none"/>
        <path d="M7 10l2 2 4-4" stroke="var(--sq-brand)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
    title: 'Accesibilidad',
  ...
  },
  {
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <path d="M3 10h14M10 3v14" stroke="var(--sq-brand)" strokeWidth="1.5" strokeLinecap="round"/>
        <circle cx="10" cy="10" r="3" fill="var(--sq-brand)" opacity="0.2" stroke="var(--sq-brand)" strokeWidth="1.5"/>
      </svg>
    ),
    title: 'Escalabilidad',
  ...
  },
  {
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <path d="M5 7l-3 3 3 3M15 7l3 3-3 3" stroke="var(--sq-brand)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M12 4l-4 12" stroke="var(--sq-brand)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
      </svg>
    ),
    ...
    <div className="docs-content">
      <div className="docs-main">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <p style={{ fontFamily: 'var(--sq-font-body)', fontSize: 15, lineHeight: 1.7, color: 'var(--sq-text-subtle)' }}>
          ...
          <div className="intro-stats-grid">
            {[
              ...
              <div key={label} style={{
                ...
                <span style={{ fontFamily: 'var(--sq-font-body)', fontSize: 28, fontWeight: 700, color: 'var(--sq-brand)', letterSpacing: '-0.03em' }}>{num}</span>
                <span style={{ fontFamily: 'var(--sq-font-body)', fontSize: 14, fontWeight: 600, color: 'var(--sq-text-default)' }}>{label}</span>
                <span style={{ fontFamily: 'var(--sq-font-body)', fontSize: 12, color: 'var(--sq-text-subtlest)', lineHeight: 1.5 }}>{sub}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="intro-principles-grid">
            {PRINCIPLES.map(({ icon, title, desc }) => (
              <div key={title} style={{
                ...
                <p style={{ fontFamily: 'var(--sq-font-body)', fontSize: 14, fontWeight: 600, color: 'var(--sq-text-default)', margin: 0 }}>{title}</p>
                <p style={{ fontFamily: 'var(--sq-font-body)', fontSize: 13, lineHeight: 1.6, color: 'var(--sq-text-subtle)', margin: 0 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <tbody>
                {STACK.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--sq-border-default)' }}>
                    ...
                    <td style={{ padding: '10px 12px', color: 'var(--sq-text-default)', fontWeight: 500 }}>{row.role}</td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <p style={{ fontFamily: 'var(--sq-font-body)', fontSize: 13, color: 'var(--sq-text-subtlest)', lineHeight: 1.6 }}>
            Los componentes en <code style={{ fontFamily: 'var(--sq-font-mono)', fontSize: 12, color: 'var(--sq-brand)', background: 'rgba(0,95,219,0.06)', padding: '1px 5px', borderRadius: 4 }}>components/ui/</code> son importados por el sitio docs mediante el alias de webpack <code style={{ fontFamily: 'var(--sq-font-mono)', fontSize: 12, color: 'var(--sq-brand)', background: 'rgba(0,95,219,0.06)', padding: '1px 5px', borderRadius: 4 }}>@squadness/ui</code>, sin necesidad de publicarlos en npm.
          </p>
        </div>
