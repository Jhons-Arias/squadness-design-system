...
const PATTERNS = [
  {
    ...
    code: `border: 1px solid var(--color-border-subtle);
  ...
  },
  {
    ...
    preview: (
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-text-subtle)' }}>Sección A</span>
        <hr style={{ border: 'none', borderTop: '1px solid rgba(11,18,14,0.14)', width: '100%' }} />
        <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-text-subtle)' }}>Sección B</span>
      </div>
    ),
    code: `border: none;
      ...
      <div className="docs-main">
            ...
            {BORDER_COLORS.map(row => (
              <div key={row.token} style={{
                ...
                <p style={{
                  ...
                  color: 'var(--color-text-subtle)', lineHeight: 1.6,
                  margin: 0,
                }}>{row.use}</p>
              </div>
            ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <Desc>
              El sistema usa exclusivamente <code style={{ fontFamily: 'var(--font-mono)', fontSize: 15, color: 'var(--color-brand)', background: 'rgba(0,95,219,0.06)', padding: '1px 6px', borderRadius: 4 }}>1px</code> en todos los bordes. Para el anillo de foco (accesibilidad) se usa <code style={{ fontFamily: 'var(--font-mono)', fontSize: 15, color: 'var(--color-brand)', background: 'rgba(0,95,219,0.06)', padding: '1px 6px', borderRadius: 4 }}>outline: 3px</code> en lugar de border para no alterar el layout.
            </Desc>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--color-brand)', margin: 0 }}>border: 1px</p>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--color-text-muted)', margin: '2px 0 0' }}>Separación y contenedores</p>
                </div>
              </div>
            </div>
            {/* outline 3px */}
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--color-brand)', margin: 0 }}>outline: 3px</p>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--color-text-muted)', margin: '2px 0 0' }}>Foco — accesibilidad</p>
                </div>
              </div>
            </div>
          </div>
            ...
            {PATTERNS.map(p => (
              <div key={p.name} style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 14, color: 'var(--color-text-primary)' }}>{p.name}</span>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-text-muted)' }}>{p.description}</span>
                </div>
                {/* Preview */}
                <div style={{ padding: '24px 20px', background: 'var(--color-surface)' }}>
                  {p.preview}
                </div>
                {/* Code */}
                <div className="docs-code-wrap">
